  ...
  <div class="home-page">
    ...
    <section class="audience-section content-width" aria-labelledby="audience-heading">
      ...
      <div class="audience-grid">
        <RouterLink class="audience-item glass-card" to="/guides"><span class="audience-icon">01</span><span class="audience-copy"><b>我还没有服务器</b><small>从 Java、服务端到第一次启动，一步一步开始。</small></span><span class="audience-arrow">→</span></RouterLink>
        <RouterLink class="audience-item glass-card" to="/tutorials"><span class="audience-icon">02</span><span class="audience-copy"><b>我已经有服务器</b><small>继续学习插件、权限、配置与日常维护。</small></span><span class="audience-arrow">→</span></RouterLink>
        <RouterLink class="audience-item glass-card" to="/plugins"><span class="audience-icon">03</span><span class="audience-copy"><b>我的服务器遇到了问题</b><small>从日志、报错、网络和性能方向快速排查。</small></span><span class="audience-arrow">→</span></RouterLink>
        <RouterLink class="audience-item glass-card" to="/tools"><span class="audience-icon">04</span><span class="audience-copy"><b>浏览知识库</b><small>教程、插件资料与常见问题都在这里。</small></span><span class="audience-arrow">→</span></RouterLink>
      </div>
    </section>
    ...
    <section class="path-section">
      <div class="content-width path-inner">
        ...
        <div class="path-track glass-card">
    ...
    <section class="knowledge-section content-width">
      ...
      <div class="knowledge-layout">
        <div class="knowledge-intro glass-card"><span class="knowledge-index">MCHIVE / LIBRARY</span><h3>从基础部署，<br />到稳定运营。</h3><p>把分散的配置说明、插件资料和实战经验，整理成能查找、能跟进的知识体系。</p><RouterLink to="/tutorials">探索知识库 <span>↗</span></RouterLink></div>
        <div class="category-list"><RouterLink v-for="item in categories" :key="item.name" class="category-row glass-card-inset" :to="item.path || `/guide/${item.guide}`"><span class="category-num">{{ item.icon }}</span><span class="category-copy"><b>{{ item.name }}</b><small>{{ item.detail }}</small></span><span class="category-count">{{ item.path ? `${pluginIndex.length} 个资料` : '查看指南' }}</span><span class="category-arrow">↗</span></RouterLink></div>
      </div>
    </section>
    ...
    <section class="latest-section content-width">
      ...
      <div class="latest-list glass-card"><RouterLink v-for="(guide, index) in guideIndex.slice(0, 3)" :key="guide.id" class="latest-row" :to="`/guide/${guide.id}`"><span class="latest-number">0{{ index + 1 }}</span><span class="latest-main"><b>{{ guide.name }}</b><small>{{ guide.description }}</small></span><span class="latest-tags"><i v-for="tag in (guide.tags || []).slice(0, 2)" :key="tag">{{ tag }}</i></span><span class="latest-arrow">↗</span></RouterLink></div>
    </section>
    ...
    <section class="plugins-section">
      <div class="content-width plugins-inner">
        ...
        <div class="plugin-preview-list glass-card"><RouterLink v-for="plugin in pluginIndex" :key="plugin.id" :to="`/plugin/${plugin.id}`" class="plugin-preview"><span class="plugin-monogram">{{ plugin.name.slice(0, 1) }}</span><span class="plugin-preview-copy"><b>{{ plugin.name }}</b><small>{{ plugin.category }} · {{ plugin.version }}</small></span><span class="plugin-preview-arrow">↗</span></RouterLink></div>
      </div>
    </section>

    <section class="contribute-section content-width glass-card"><div><span class="section-kicker">BUILT WITH THE COMMUNITY</span><h2>让开服知识，<br class="mobile-break" />一起变得更好。</h2><p>MCHive 是开放的社区项目。分享教程、补充配置说明，或修正文档中的错误，都能帮助下一位服主少走弯路。</p></div><a class="button-primary" href="https://github.com/MCTranslate/MCHive" target="_blank" rel="noreferrer">参与 MCHive <span>↗</span></a></section>
  </div>
</template>
...
@keyframes dotPulse {
  0%, 100% { box-shadow: 0 0 0 4px var(--accent-dim), 0 0 12px var(--accent); }
  50% { box-shadow: 0 0 0 6px var(--accent-dim), 0 0 20px var(--accent-hover); }
}
.hero-copy h1 {
  margin-top: 24px;
...
.hero-highlight {
  background: linear-gradient(135deg, var(--accent) 0%, #a7f3d0 50%, var(--accent-hover) 100%);
...
.hero-search:focus-within {
  ...
  box-shadow: 0 0 0 3px var(--accent-dim), 0 8px 32px rgba(110, 231, 183, 0.1);
}
...
.button-primary {
  ...
  background: linear-gradient(135deg, var(--accent), var(--accent-hover));
  color: var(--button-text);
  box-shadow: 0 2px 16px rgba(110, 231, 183, 0.25);
}
...
.hero-metrics i { width: 4px; height: 4px; border-radius: 50%; background: var(--text-muted); opacity: 0.5; }
...
.node small { margin-top: 3px; color: var(--text-muted); font: 8px var(--font-mono); letter-spacing: 0.5px; }
.node i {
  position: absolute;
  right: 10px;
  top: 8px;
...
.node-core .node-glyph {
  ...
  box-shadow: 0 2px 12px rgba(110, 231, 183, 0.4);
}
...
.audience-arrow { color: var(--accent); font-size: 18px; opacity: 0.5; transition: opacity 0.2s; }
.audience-item:hover .audience-arrow { opacity: 1; transform: translateX(4px); }
...
.inline-link span { margin-left: 8px; transition: transform 0.2s; display: inline-block; }
...
.step-top span {
  ...
  font-weight: 600;
  box-shadow: 0 2px 10px rgba(110, 231, 183, 0.15);
}
.step-top i {
  position: absolute;
  top: 14px;
...
.knowledge-intro > a:hover { gap: 8px; }
...
.category-num { color: var(--accent); font: 11px var(--font-mono); opacity: 0.6; }
.category-copy { display: flex; flex: 1; flex-direction: column; gap: 4px; }
.category-copy b { font-size: 13px; font-weight: 620; }
.category-copy small, .category-count { color: var(--text-muted); font-size: 10px; }
.category-count { white-space: nowrap; }
.category-arrow { color: var(--text-muted); font-size: 14px; transition: transform 0.2s; }
.category-row:hover .category-arrow { transform: translateX(4px); }
...
.latest-arrow { color: var(--text-muted); transition: transform 0.2s; }
.latest-row:hover .latest-arrow { transform: translateX(4px); }
...
.plugins-inner { display: grid; grid-template-columns: 1fr 1fr; gap: 42px; align-items: center; }
...
.plugin-heading .button-outline:hover {
  background: var(--accent-dim);
  border-color: var(--accent);
}
...
.plugin-preview-copy small { color: var(--text-muted); font-size: 10px; }
.plugin-preview-arrow { color: var(--text-muted); transition: transform 0.2s; }
.plugin-preview:hover .plugin-preview-arrow { transform: translateX(4px); }
...
@media (max-width: 700px) {
  ...
  .hero-visual {
    width: 100%; max-width: none; height: 240px;
    margin-top: 28px; transform: none;
  }
  ...
  .node-java { left: 0; top: 5%; }
  .node-paper { right: 0; top: 5%; }
  .node-plugin { left: 0; bottom: 0; }
  .node-world { right: 0; bottom: 0; }
  ...
  .audience-copy b { font-size: 13px; }
  .audience-copy small { font-size: 10px; }
  ...
  .plugins-inner { grid-template-columns: 1fr; gap: 28px; }
