...
<template>
  <div class="search-page content-width">
    <nav class="breadcrumb" aria-label="面包屑">
      <RouterLink to="/">首页</RouterLink><span>/</span><span>搜索</span>
    </nav>
    <header class="search-header">
      <span class="section-kicker">SEARCH THE KNOWLEDGE BASE</span>
      <h1>搜索结果</h1>
      <p v-if="query">“{{ query }}” 找到 {{ results.length }} 条相关内容</p>
      <p v-else>输入教程名称、插件或遇到的问题。</p>
    </header>
    ...
    <div v-else class="search-empty">
      <span class="empty-kicker">{{ query ? 'NO RESULTS' : 'SEARCH MCHIVE' }}</span>
      <h2>{{ query ? '暂时没有找到匹配内容。' : '这里可以找到什么？' }}</h2>
      <p>{{ query ? '可以尝试缩短关键词，或使用插件名、配置项和问题中的核心词语。' : '教程、插件资料、配置说明和常见问题都可以从顶部搜索。' }}</p>
      <RouterLink to="/tutorials" class="inline-link">浏览全部教程 <b>→</b></RouterLink>
    </div>
  </div>
</template>
...
.result-row {
  display: flex; align-items: center; min-height: 88px; gap: 18px; padding: 16px;
...
@media (max-width: 700px) {
  ...
  .search-header { margin-top: 32px; padding-bottom: 18px; }
